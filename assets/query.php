...
<?php
   ...
   if(isset($_POST['getBuildingNames'])) {
     $STH = $conn->query("SELECT Building.BuildingName AS 'Building Name' FROM Building WHERE Building.CampusID='".$_POST['CampID']."' ");
?>
<div class="panel-body">
   <div class="modal fade" id="listOfCampus" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered" role="document">
...
<?php
                     while($row = $STH->fetch()) {
                       echo "<option name='display' value='".$row['BuildingName']."'>".$row['BuildingName']."</option>";
                     }
?>
                  </select>
                  <div class="modal-footer">
